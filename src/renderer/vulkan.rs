...
use vulkano::{instance::{Instance, InstanceCreateInfo}, swapchain::{Surface, Swapchain, SwapchainCreateInfo, SwapchainCreationError, self, AcquireError, PresentFuture, SwapchainAcquireFuture, PresentInfo}, device::{Device, Queue, DeviceExtensions, QueueCreateInfo, DeviceCreateInfo}, image::{SwapchainImage, ImageUsage, view::ImageView}, render_pass::{RenderPass, Framebuffer, FramebufferCreateInfo, Subpass}, shader::ShaderModule, command_buffer::{PrimaryAutoCommandBuffer, AutoCommandBufferBuilder, CommandBufferUsage, RenderPassBeginInfo, SubpassContents, CommandBufferExecFuture}, pipeline::{graphics::{viewport::{Viewport, ViewportState}, vertex_input::BuffersDefinition, input_assembly::InputAssemblyState}, GraphicsPipeline}, sync::{FenceSignalFuture, self, FlushError, JoinFuture, GpuFuture}, VulkanLibrary, buffer::{CpuAccessibleBuffer, TypedBufferAccess, BufferUsage}};
use vulkano_win::VkSurfaceBuild;
use winit::{window::{Window, WindowBuilder}, event_loop::{EventLoop, ControlFlow}, event::{Event, WindowEvent}, platform::run_return::EventLoopExtRunReturn};
...
fn add_actor_buffers(builder: &mut AutoCommandBufferBuilder<PrimaryAutoCommandBuffer>, actors: &Vec<&Actor>) {
  for actor in actors {
