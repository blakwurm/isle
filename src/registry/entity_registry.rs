use std::{any::{TypeId, Any}, collections::{HashMap, HashSet}, hash::Hash};
...
impl EntityRegistry {
  ...
  pub fn get_entities_by_components(&self, components: Vec<TypeId>) -> Option<HashSet<String>> {
    let mut set: HashSet<String> = self.components.get(&components[0])?.iter().cloned().collect();
...
mod entity_registry_tests {
  ...
  fn test_component_add() {
    ...
    assert_eq!(registry.get_component::<i32>(&String::from("test_entity")), Some(&1));
    assert_eq!(registry.get_component::<i64>(&String::from("test_entity")), Some(&2_i64));
  }
  ...
  fn test_get_entities_by_component() {
    ...
    assert_eq!(registry.get_entities_by_component::<i32>(), Some(vec!(&String::from("test_entity_1"))));
    assert_eq!(registry.get_entities_by_component::<i64>(), Some(vec!(&String::from("test_entity_2"))));
  }
  ...
  fn test_get_entities_by_components() {
    ...
    let entities = registry.get_entities_by_components(vec![TypeId::of::<i32>(), TypeId::of::<i64>()]).unwrap();
